...
- **Independent Swift menu-bar app** — Ratko runs separately from Obsidian, stays available while Obsidian reloads or is closed, and reads the same Markdown tasks directly. Its menu-bar icon, panel header, and focus window use TaskMaster's original full-color Ratko artwork; the visually dense menu-bar image renders at 14 points to match neighboring line icons. On every launch it restores its status-item preference immediately to the left of macOS Wi-Fi so a crowded menu bar does not park it off-screen. Click it to see active work and elapsed time, add, edit, or reorder steps without losing their timers, write a memo, create a task, or start a waiting task. Visible task handles reorder every current or upcoming card within its list or across both lists, and `.board.json` restores that mixed-status order after relaunch. Drag the visible grip along the panel's bottom edge to resize the task viewport; Ratko remembers the chosen height and clamps it to the usable screen.
