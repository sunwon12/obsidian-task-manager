...
- **Drag between 현재 작업 and 다음 할 일** — drag a card's background, status, timer, or other empty surface to freely reorder all current and upcoming tasks, or cross the section boundary. Task titles, steps, inputs, and action buttons remain ordinary click targets. Dropping a waiting task onto the focus section moves it to DOING and starts or resumes its timer; dragging a focused card down moves it to TODO and pauses it without losing total or per-step elapsed time. The order survives app and Obsidian restarts, and the plugin re-reads the Swift-owned order before any board write so a stale in-memory board cannot overwrite a recent drag.
