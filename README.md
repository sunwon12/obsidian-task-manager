...
- **Independent Swift menu-bar app** — Ratko runs separately from Obsidian, stays available while Obsidian reloads or is closed, and reads the same Markdown tasks directly. Its menu-bar icon, panel header, and focus window use TaskMaster's original full-color Ratko artwork; the visually dense menu-bar image renders at 14 points to match neighboring line icons. On every launch it restores its status-item preference immediately to the left of macOS Wi-Fi so a crowded menu bar does not park it off-screen. Click it to see active work and elapsed time, add, edit, or reorder steps without losing their timers, write a memo, create a task, or start a waiting task. Dragging any noninteractive card surface reorders current or upcoming cards within a list or across both lists, while titles, steps, inputs, and action buttons keep their click behavior; `.board.json` restores that mixed-status order after relaunch. Drag the visible grip along the panel's bottom edge to resize the task viewport; Ratko remembers the chosen height and clamps it to the usable screen.
...
- **Drag between 현재 작업 and 다음 할 일** — drag a card's background, status, timer, or other empty surface to freely reorder all current and upcoming tasks, or cross the section boundary. Task titles, steps, inputs, and action buttons remain ordinary click targets. Holding the pointer near the top or bottom of the task viewport continuously auto-scrolls the list, with speed increasing toward the edge. Dropping a waiting task onto the focus section moves it to DOING and starts or resumes its timer; dragging a focused card down moves it to TODO and pauses it without losing total or per-step elapsed time. The order survives app and Obsidian restarts, and the plugin re-reads the Swift-owned order before any board write so a stale in-memory board cannot overwrite a recent drag.
